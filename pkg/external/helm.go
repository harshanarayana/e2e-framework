...
func (m *helmManager) processOpts(opts ...Option) *Opts {
	option := &Opts{}
	for _, op := range opts {
		op(option)
	}
...
func (m *helmManager) getCommand(opt *Opts) (string, error) {
...
func (m *helmManager) RunTemplate(opts ...Option) error {
	o := m.processOpts(opts...)
	o.mode = "template"
	return m.run(o)
}
...
func (m *helmManager) run(opts *Opts) (err error) {
...
func NewHelmManager(kubeConfig string) *helmManager {
	return &helmManager{e: gexe.New(), kubeConfig: kubeConfig}
}
