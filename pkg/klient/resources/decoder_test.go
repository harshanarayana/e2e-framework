...
func TestDecodeFile(t *testing.T) {
	...
	cfg := v1.ConfigMap{}
	if err := DecodeFile(testdata, testYAML, &cfg, MutateOption(func(o types.Object) error {
...
func TestDecodeEachFile(t *testing.T) {
	...
	count := 0
	if err := DecodeEachFile(context.TODO(), testdata, serviceAccountPrefix, func(ctx context.Context, obj types.Object) error {
		count++
		return nil
	}); err != nil {
		t.Fatal(err)
	...
	serviceAccounts := 0
	configs := 0
	if err := DecodeEachFile(context.TODO(), testdata, "*", func(ctx context.Context, obj types.Object) error {
		count++
		switch obj.(type) {
		case *v1.ConfigMap:
			configs++
...
func TestDecodeEach(t *testing.T) {
	...
	defer f.Close()
	count := 0
	err = DecodeEach(context.TODO(), f, func(ctx context.Context, obj types.Object) error {
		count++
		switch cfg := obj.(type) {
		case *v1.ConfigMap:
...
func TestHandlerFuncs(t *testing.T) {
	...
	t.Run("DecodeEach_ReadHandler", func(t *testing.T) {
		...
		serviceAccounts := 0
		configs := 0
		if err := DecodeEachFile(context.TODO(), testdata, "*", ReadHandler(res, func(ctx context.Context, obj types.Object) error {
			if labels := obj.GetLabels(); labels["injected"] != testLabel {
				t.Fatalf("unexpected value in labels: %q", labels["injected"])
			} else {
				count++
	...
	t.Run("DecodeEach_Delete", func(t *testing.T) {
		...
		t.Run("Verify", func(t *testing.T) {
			count := 0
			for i := range objects {
				if err := IgnoreErrorHandler(ReadHandler(res, func(ctx context.Context, obj types.Object) error {
