...
func setup() {
	...
	if os.IsNotExist(err) {
		...
		if err != nil {
			log.Info("failed to create .kube dir", err)
			return
		}
		...
		if err != nil {
			log.Info("failed to create config file", err)
			return
		}
	}
	...
	if err != nil {
		log.Info("unexpected error while setting flag value", err)
		return
	}
...
func teardown() {
	...
	if err != nil {
		log.Info("failed to delete .kube dir", err)
	}
}
