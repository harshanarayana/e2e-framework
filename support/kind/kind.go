...
func (k *Cluster) CreateWithConfig(imageName, kindConfigFile string) (string, error) {
	log.Info("Creating kind cluster ", k.name)
	if err := k.findOrInstallKind(k.e); err != nil {
		return "", err
	}

	if strings.Contains(k.e.Run("kind get clusters"), k.name) {
		log.Info("Skipping Kind Cluster.Create: cluster already created: ", k.name)
		return k.getKubeconfig()
	}

	log.Info("launching: kind create cluster --name", k.name, "--image", imageName, "--config", kindConfigFile)
	...
	log.Info("kind clusters available: ", clusters)
...
func (k *Cluster) Create() (string, error) {
	...
	log.Info("launching: kind create cluster --name", k.name)
...
func (k *Cluster) Destroy() error {
	log.Info("Destroying kind cluster ", k.name)
	if err := k.findOrInstallKind(k.e); err != nil {
		return err
	}
	...
	log.Info("Removing kubeconfig file ", k.kubecfgFile)
...
func (k *Cluster) findOrInstallKind(e *gexe.Echo) error {
	if e.Prog().Avail("kind") == "" {
		log.Info(`kind not found, installing with GO111MODULE="on" go get sigs.k8s.io/kind@v0.11.0`)
...
func (k *Cluster) installKind(e *gexe.Echo) error {
	...
	log.Info("installing: go get sigs.k8s.io/kind@", kindVersion)
	...
	if kindPath := e.Prog().Avail("kind"); kindPath != "" {
		log.Info("installed kind at", kindPath)
		return nil
	}
	...
	log.Info(`Setting path to include $GOPATH/bin:`, p.Result())
